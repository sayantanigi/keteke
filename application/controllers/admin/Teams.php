...
class Teams extends Admin_Controller {
	...
	public function add($id=false)
	{
		...
		$this->form_validation->set_rules('frm[name]', 'Member title', 'required');
		if ($this->form_validation->run()) {
			...
			$this->session->set_flashdata("success", "Team saved successfully");
    ...
    {
        $redirect = isset($_GET['redirect_to']) ? $_GET['redirect_to'] : admin_url('teams');
        if ($id) {
            $c['id'] = $id;
            $c['status'] = 1;
            $this->Teams_model->save($c);
            $this->session->set_flashdata("success", "Team activated");
        }
        redirect($redirect);
    }
    ...
    function deactivate($id = false)
    {
        ...
        if ($id) {
            ...
            $c['status'] = 0;
            $this->Teams_model->save($c);
            $this->session->set_flashdata("success", "Team deactivated");
        }
        redirect($redirect);
    }
