<?php
class Option_model extends Master_model{
    var $_global;
	...
	function all_options(){
		$data = array();
		$rest = $this -> db -> get('whychooseus') -> result_array();
    ...
    function get_option($option_name){
        $d = $this -> db -> get_where('whychooseus', array('option_name' => $option_name)) -> first_row('array');
        if(is_array($d)){
            return $d;
        }else{
	...
	function save_option($data){
		$rest = $this -> db -> get_where('whychooseus', array('option_name' => $data['option_name']));
		if($rest -> num_rows() > 0){
			$this -> db -> where('option_name', $data['option_name']);
			$this -> db -> update('whychooseus', $data);
		}else{
			$this -> db -> insert('whychooseus', $data);
		}
	}
