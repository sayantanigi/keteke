            ...
            <table class="table table-bordered tablefontsmall">
              ...
              if(is_array($ordrs) && count($ordrs)>0){
                ...
                foreach ($ordrs as $ord_v) {
                  ...
                  $primg=$this->Master_model->getSingleRow('productId',$ord_v->product_id,'product_images');

                  $shaddr=$this->Master_model->getSingleRow('order_id',$ord_v->draftpayment_orderid,'customer_shipping_addrs');
                  
                  $baddr=$this->Master_model->getSingleRow('order_id',$ord_v->draftpayment_orderid,'customer_billing_addrs');
                  ...
                  <tr>
                    ...
                    <td><img src="<?=site_url('assets/images/products/'.@$primg->productImage)?>"  width="80px" onerror="this.src='<?=site_url('assets/images/no-image.png');?>';">
                      ...
                      <td><p>Offer Price: <?=$ord_v->offprice?> USD<br>
                        Actual Price: <?=$ord_v->maxPrice?> USD<br>
                        Disc %: <?=$ord_v->disc_percent?></p>
                      </td>
                      <td><?=@$mcatt->name?></td>
                      <td><?=@$ord_v->brand_name?></td>
                      <td><?=@$ord_v->order_status?></td>
                      <td><?=date('d M Y',strtotime($payInfo->tran_date))?></td>
                    </tr>
                    <?php
                    $i++;
                  }
             ...
             <a class="btn btn-success text-right" href="<?=admin_url('products/DraftOrdertransactions')?>">Back to draft Order lists</a>
           </div>
           <!-- /.box-body -->
         </div>
       </div>
