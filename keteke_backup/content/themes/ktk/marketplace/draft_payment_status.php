  ...
  <div class="col-lg-10 offset-lg-1">
    <div class="boxPaypent">
      ...
      <table class="table-bordered table">
        ...
        <tbody>
           <?php
            ...
            if(is_array($draftproducts) && count($draftproducts)>0){
              $i=0;
          foreach ($draftproducts as $prd) { 
            $i++;
            $prdetl=$this->Master_model->getSingleRow("productId",$prd->product_id, "products");
             $shaddr=$this->Master_model->getSingleRow('order_id',$prd->draftpayment_orderid,'customer_shipping_addrs');
                  
            $baddr=$this->Master_model->getSingleRow('order_id',$prd->draftpayment_orderid,'customer_billing_addrs');

            ?>
          <tr>
            <td><?=$i?></td>
            <td><?=@$prdetl->productName?></td>
            <td><?=@$prd->qty?></td>
            <td><?=$prd->qty.'*'.$prd->price?> USD</td>
            <td><?=$prd->shipping_charge?> USD</td>
      ...
      <div class="text-center">
        <a href="<?=site_url('shop')?>" class="btn btn-danger" style="color: #fff !important;">Thank You</a>
      </div>
    </div>
  </div>
</div>
