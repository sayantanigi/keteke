                ...
                <div class="col-sm-12 col-md-12 col-xl-6 col-lg-5 ">
                    <div class="search">
                        <div class="form-box">
                            ...
                            <h2><span <?php if ($uri == 'login') { ?> class="sactive" <?php } else { ?>class="in-active" <?php } ?> style="background:transparent;box-shadow:none;" id="login-name"
                                    onclick="openlogin();">Login</span> &emsp; <span class="divider"> | &emsp;</span>
                                <span <?php if ($uri == 'signup') { ?> class="sactive" <?php } else { ?>class="in-active"
                                    <?php } ?> id="signup-name" onclick="opensignup();">Signup</span>
                            </h2>
                            <br>
                            <br>
                            <?php
                            ...
                            ?>
                            <div class="login" id="login" <?php if ($uri == 'login') { ?> style="display: block;" <?php } ?>
                                <?php if ($uri == 'signup') { ?> style="display: none;" <?php } ?>>
                                <form class="animate__animated animate__backInLeft" role="form"
                                    action="<?= site_url('login') ?>" method="post" id="log-form">
                                    <p id="text" class="capstext">Email address and paswword are case sentive.WARNING!
                                        Caps lock is On.</p>
                                    <div class="form-item">
                                        <i class="fa fa-envelope" aria-hidden="true"></i>
                                        <input type="email" name="umail" id="validemail" placeholder="Email"
                                            autocomplete="off" required>
                                    </div>
                                    <div class="form-item">
                                        <i class="fa fa-lock" aria-hidden="true"></i>
                                        <input type="password" name="upass" id="validpassword" value=""
                                            placeholder="Password" required>
                                    </div>
                                    <p class="almem"><a href="<?= site_url('forgot-password') ?>">Forgot Password?</a></p>
                                    <div class="form-item-submit">
                                        <input type="submit" value="Login">
                                        <input type="button" onclick="goBack()" value="Cancel" style="background: red;color: white; padding: 10px 30px; border-radius: 50px; font-size: 16px; border: none; box-shadow: rgba(0, 0, 0, 0.18) 0px 3px 6px !important;">
                                    </div>
                                </form>
                            </div>
                            <div class="login" id="signup" <?php if ($uri == 'signup') { ?> style="display: block;" <?php } ?> <?php if ($uri == 'login') { ?> style="display: none;" <?php } ?>>
                                <form class="animate__animated animate__backInRight" role="form"
                                    action="<?= site_url('signup') ?>" method="post" id="sig-form"
                                    ...
                                    <div class="form-item">
                                        ...
                                        <input type="password" id="confirm_password" name="con_password"
                                            onchange="check_pass()" placeholder="Confirm Password" required>
                                    </div>
                                    <div class="form-item">
