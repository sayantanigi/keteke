...
class Welcome extends AI_Controller {
    ...
    public function index() {
        ...
        if ($query && $query['status'] == 'success') {
            ...
            $searchSql = "SELECT *, ( 6367 * acos( cos( radians('" . $lat . "') ) * cos( radians( `lati` ) ) * cos( radians( `longi` ) - radians('" . $lon . "') ) + sin( radians('" . $lat . "') ) * sin( radians( `lati` ) ) ) ) AS distance FROM `listing` having (distance BETWEEN 0 AND 5000 OR distance IS NULL) AND (status = '1' OR prefer_list = '1' ) ORDER BY  distance ASC LIMIT 3";
    ...
    public function search_list() {
        ...
        if ($scat != '') {
            ...
            if ($loc != "" && $scat != '') {
                ...
                $searchSql = "SELECT listing.id, listing.userid, listing.title, listing.busi_classi, listing.other_classi, listing.street_addr, listing.city, listing.country, listing.category, listing.subcategory, listing.keywords, listing.email, listing.phone, listing.images, listing.lati, listing.longi, listing.status, category.name, user_accounts.user_fname, user_accounts.user_lname, (6367 * acos(cos(radians($latitude)) * cos(radians(listing.lati)) * cos(radians(listing.longi) - radians($longitude)) + sin(radians($latitude)) * sin(radians(listing.lati)))) AS distance FROM listing JOIN category ON listing.category = category.id JOIN user_accounts ON listing.userid = user_accounts.user_id AND listing.status = '1' AND (listing.category = '".$scat."' OR listing.category = '%".$scat."%' OR category.name LIKE '%".$scat."%') ORDER BY distance ASC";
    ...
    function contact() {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($res1 == true) {
                ...
                //$htmlContent = "<body style='background-color:#e2e1e0;font-family: Open Sans, sans-serif;font-size:100%;font-weight:400;line-height:1.4;color:#000;'><table style='max-width:600px;margin:20px auto 10px;background-color:#fff;padding:20px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px;-webkit-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);-moz-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24); border-top: solid 10px #000000;'><thead><tr><th style='text-align:left;'><img style='max-width: 150px;' src='" . site_url('fassets/images/logos/headertransparentlogo.png') . "' alt='Keteke'></th><th style='text-align:right;font-weight:400;'>" . date('d M Y') . "</th></tr></thead><tbody><tr><td colspan='2' style='height:2px;background:#232323'></td></tr><tr><td colspan='2' style='padding:20px 0'><h3 style='margin;0;color:green'>Dear Admin,</h3><p>A new Enquiry message for Keteke. Kindly check the below complete information of the Candidate.</p></td></tr><tr><td style='height:35px;'><h4 style='margin:0 '>Enquiry Detail</h4></td></tr><tr><td style='width:50%;padding:0;vertical-align:top'><p style='margin:0 0 10px 0;padding:0;font-size:14px;'><span style='display:block;font-weight:bold;font-size:13px'>Name</span> " . $formdata['name'] . "</p><p style='margin:0 0 10px 0;padding:0;font-size:14px;'><span style='display:block;font-weight:bold;font-size:13px;'>Email</span> " . $formdata['email'] . "</p><p style='margin:0 0 10px 0;padding:0;font-size:14px;'><span style='display:block;font-weight:bold;font-size:13px;'>Subject</span> " . $formdata['subject'] . "</p></td></tr><tr><td style='width:50%;padding:0;vertical-align:top'><p style='margin:0 0 10px 0;padding:0;font-size:14px;'><span style='display:block;font-weight:bold;font-size:13px;'>Message</span> " . $formdata['message'] . "</p></td></tr><tr><td style='height:15px;'></td></tr></tbody></table></body>";

                $htmlContent = "<div style='width:600px;margin: 0 auto;background: #fff;font-family: 'Poppins', sans-serif; border: 1px solid #e6e6e6;'> <div style='padding: 30px 30px 15px 30px;box-sizing: border-box;'> <img src='" . site_url('fassets/images/logos/headertransparentlogo.png') . "' style='width:100px;float: right;margin-top: 0 auto;'> <h3 style='padding-top: 40px;line-height: 20px;font-weight: 100;font-size: 15px;'>Greetings from<span style='font-weight: 900;font-size: 23px;color: #e61d25;display: block;'>Keteke</span></h3> <p style='font-size: 15px;'>Hello Admin,</p> <p style='font-size: 15px;'>A new enquiry message for Keteke. Kindly check the below complete information of the Candidate.</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Name: ".$_POST['name']."</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Email: ".$_POST['email']."</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Subject: ".$_POST['subject']."</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Message: ".$_POST['message']."</p> <p style='font-size: 15px; padding: 0; margin: 18px 0 0 0;'>Thank you!</p> <p style='font-size: 15px; padding: 0; margin: 0; list-style: none;'>Sincerly,</p> <p style='font-size: 15px; list-style: none; padding: 0; margin: 0;'><b>Keteke</b></p> </div> <table style='width: 100%;'> <tr> <td style='height:30px;width:100%; background: #e61d25 ;padding: 10px 0px; font-size:13px; color: #fff; text-align: center;'>Copyright &copy; <?=date('Y')?> Keteke. All rights reserved.</td> </tr> </table> </div>";
    ...
    public function login_ajax() {
        ...
        if ($this->form_validation->run()) {
            ...
            /*$sql = "SELECT * FROM `user_accounts` WHERE (user_emailid = '$mail' AND user_pasword='$pass') AND user_status = '1' AND email_verified = '1'";
            $check = $this->db->query($sql)->num_rows();
            $user = $this->db->query($sql)->row();*/
            $user = $this->db->query("SELECT * FROM `user_accounts` WHERE user_emailid = '".$mail."' AND user_pasword = '".$pass."'")->row();
            ...
            if (!empty($user)) {
                if($user->user_status == '0') {
                    ...
                    $this->session->set_flashdata('error', 'Your account is not email verified yet. Please verify your email then try to login.');
                } else {
                    ...
                    $checkAddtoCart = $this->db->query("SELECT * FROM add_to_cart WHERE session_id = '".session_id()."'")->result_array();
                    if(!empty($checkAddtoCart)) {
                        $updatecart = $this->db->query("UPDATE add_to_cart SET user_id = '".$user->user_id."' WHERE session_id = '".session_id()."'");
                    }
    ...
    function reset_pass() {
        ...
        $name = $user->fname;
        $htmlContent = "<table align='center' style='width:650px; text-align:center; background:#8e88881f;'> <tbody> <tr style='height:50px;background-color:#ffeabf;'> <td valign='middle' style='color:white;'><img src='" . base_url() . "fassets/images/logo.png' alt='Keteke' title='Keteke Logo' height='100px' /></td> </tr> <tr> <td valign='top' align='center' colspan='2'> <table align='center' style='height:380px; color:#000; width:600px;'> <tbody> <tr> <td style='width:8px;'>&nbsp;</td> <td align='center' style='font-size:28px;border-top:1px dashed #ccc;' colspan='3'>Hello, " . $name . "</td> </tr> <tr> <td valign='top' align='center' colspan='2'> <p>Your Password is " . $pass . ".</p> <br> Best Regards,<br> Keteke <br><br> This is an automated response, please DO NOT reply. </td> </tr> </tbody> </table> </td> </tr> </tbody> </table>";

        /*$htmlContent = "<body> <div style='width:600px;margin: 0 auto;background: #fff;font-family: 'Poppins', sans-serif; border: 1px solid #e6e6e6;'> <div style='padding: 30px 30px 15px 30px;box-sizing: border-box;'> <img src='".base_url()."fassets/images/logo.png' style='width:100px;float: right;margin-top: 0 auto;'> <h3 style='padding-top: 40px;line-height: 20px;font-weight: 100;font-size: 15px;'>Greetings from <span style='font-weight: 900;font-size: 23px;color: #e61d25;display: block;'>Keteke</span></h3> <p style='font-size: 15px;'>Hello ".$name.",</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Thank you for registration on Side Quote.</p> <p style='font-size: 15px; padding: 0; margin: 0;'>Please click the button below to verify your email address.</p> <p style='font-size: 15px;text-align: center;'><a href='".base_url() . "email-verification/" . urlencode(base64_encode($insert_id))."' style='height: 30px; width: 150px; background: rgb(253,179,2); background: linear-gradient(0deg, rgba(253,179,2,1) 0%, rgba(244,77,9,1) 100%); text-align: center; font-size: 15px; color: #fff; border-radius: 12px; display: inline-block; line-height: 30px; text-decoration: none; text-transform: uppercase; font-weight: 600;'>ACTIVATE</a></p> <p style='font-size: 15px;'>Thank you!</p> <p style='font-size: 15px; padding: 0; margin: 0; list-style: none;'>Sincerly</p> <p style='font-size: 15px; padding: 0; margin: 0; list-style: none;'><b>Side Quote</b></p> </div> <table style='width: 100%;'> <tr> <td style='height:30px;width:100%; background: red;padding: 10px 0px; font-size:13px; color: #fff; text-align: center;'>Copyright &copy; <?=date('Y')?> Side Quote. All rights reserved.</td> </tr> </table> </div> </body>";*/
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if (!empty($check_email)) {
                ...
                if ($res == true) {
                    ...
                    $htmlContent = "<body><div style='width:600px;margin: 0 auto;background: #fff;font-family: 'Poppins', sans-serif; border: 1px solid #e6e6e6;'><div style='padding: 30px 30px 15px 30px;box-sizing: border-box;'><img src='" . site_url('fassets/images/logos/headertransparentlogo.png')."' style='width:100px;float: right;margin-top: 0 auto;'><h3 style='padding-top: 40px;line-height: 20px;font-weight: 100;font-size: 15px;'>Greetings from <span style='font-weight: 900;font-size: 23px;color: #e61d25;display: block;'>Keteke</span></h3><p style='font-size: 15px;'>Hello ".$_POST['fname'].",</p><p style='font-size: 15px; padding: 0; margin: 0;'>Thank you for registration on Keteke.</p><p style='font-size: 15px; padding: 0; margin: 0;'>Please click the button below to verify your email address.</p><p style='font-size: 15px;text-align: center;'><a href='".site_url() . "email-verification/" . urlencode(base64_encode($insert_id))."' style='height: 30px; width: 150px; background: rgb(253,179,2); background: linear-gradient(45deg, black, #ff0000); text-align: center; font-size: 15px; color: #fff; border-radius: 12px; display: inline-block; line-height: 30px; text-decoration: none; text-transform: uppercase; font-weight: 600;'>ACTIVATE</a></p><p style='font-size: 15px;'>Thank you!</p><p style='font-size: 15px; padding: 0; margin: 0; list-style: none;'>Sincerly,</p><p style='font-size: 15px; padding: 0; margin: 0; list-style: none;'><b>Keteke</b></p></div><table style='width: 100%;'><tr><td style='height:30px;width:100%; background: red;padding: 10px 0px; font-size:13px; color: #fff; text-align: center;'>Copyright &copy; <?=date('Y')?> Keteke. All rights reserved.</td></tr></table></div></body>";
                    $headers = "MIME-Version: 1.0" . "\r\n";
                    $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
                    // More headers
                    $headers .= 'From: KETEKE Signup Page' . "\r\n";
                    //mail($email, $subject, $htmlContent, $headers);
                    //$this->session->set_flashdata('success', 'We have sent an activation link to your account to continue with the registration process. ');
                    if (mail($email, $subject, $htmlContent, $headers)) {
                        $this->session->set_flashdata('success', 'We have sent an activation link to your account to continue with the registration process. ');
                    } else {
                        $error = error_get_last();
                        $this->session->set_flashdata('error', 'Email could not be sent. Error: ' . $error['message']);
                    }
                    if($u_type == 4) {
    ...
    public function emailVerification($otp=null) {
        ...
        $sql = "SELECT * FROM `user_accounts` WHERE user_id = '".$givenotp."' AND user_status = '0' AND `email_verified` = '0'";
        $check = $this->db->query($sql)->num_rows();
        $data = array(
